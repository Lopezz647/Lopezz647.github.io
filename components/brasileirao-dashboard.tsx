    ...
    <div style={{ backgroundColor: '#09090b', color: '#fafafa', minHeight: '100vh', padding: '20px', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          ...
          <button 
            onClick={fetchData}
            style={{ backgroundColor: '#27272a', border: 'none', color: 'white', padding: '10px', borderRadius: '50%', cursor: 'pointer' }}
          >
            🔄
          </button>
        </header>
              ...
              <div key={m.id} style={{ backgroundColor: '#18181b', border: '1px solid #27272a', padding: '15px', borderRadius: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ flex: 1, textAlign: 'right', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px' }}>
                    <span style={{ fontSize: '14px' }}>{m.home}</span>
                    <img src={m.homeCrest} alt="" style={{ width: '24px', height: '24px', objectFit: 'contain' }} />
                  </div>
                  ...
                  <div style={{ width: '80px', textAlign: 'center', backgroundColor: '#27272a', padding: '5px', borderRadius: '8px', margin: '0 15px' }}>
                    ...
                    <div style={{ fontSize: '9px', fontWeight: 'bold', color: m.status === 'IN_PLAY' ? '#ef4444' : '#a1a1aa' }}>
                      {m.status === 'IN_PLAY' ? '• AO VIVO' : m.status}
                    </div>
                  </div>

                  <div style={{ flex: 1, textAlign: 'left', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <img src={m.awayCrest} alt="" style={{ width: '24px', height: '24px', objectFit: 'contain' }} />
                    <span style={{ fontSize: '14px' }}>{m.away}</span>
                  </div>
                </div>
              </div>
            ))}
